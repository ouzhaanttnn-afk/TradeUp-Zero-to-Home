...
describe("decision vertical slice", () => {
  ...
  it("adds a varied late-career equipment bridge without duplicating assets", () => {
    expect(lateCareerExpansionFamilies).toHaveLength(13);
    expect(
      lateCareerExpansionFamilies.filter((family) => family.tier === 3),
    ).toHaveLength(4);
    expect(
      lateCareerExpansionFamilies.filter((family) => family.tier === 4),
    ).toHaveLength(5);
    expect(
      lateCareerExpansionFamilies.filter((family) => family.tier === 5),
    ).toHaveLength(4);
    expect(
      new Set(lateCareerExpansionFamilies.map((family) => family.assetKey))
        .size,
    ).toBe(13);
    expect(
      new Set(lateCareerExpansionFamilies.map((family) => family.category))
        .size,
